...
`run.sh` handles everything — dependency install, auth checks, and starting both servers:
...
2. Install Node frontend dependencies (`npm install`) — skipped if already installed
3. Verify Databricks `logfood` profile and `gcloud` auth (warns if missing)
4. Start backend (`http://localhost:8000`) and frontend (`http://localhost:5173`)

Open **http://localhost:5173** in your browser.

To stop: press **Ctrl+C** (or `pkill -f uvicorn; pkill -f vite`)
